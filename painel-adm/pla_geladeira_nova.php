					...
					<div class="row">
						...
						<div class="col-md-4 col-sm-12">
							<div class="mb-4">
								...
								<select class="form-select" aria-label="Default select example" name="<?php echo $campo2 ?>" id="<?php echo $campo2 ?>">
									<?php 
									$query = $pdo->query("SELECT * FROM projetos WHERE id_plataforma = 6 ORDER BY descricao ASC");
						...
						<div class="col-md-4 col-sm-12">
							...
							<select class="form-select" aria-label="Default select example" name="<?php echo $campo1 ?>" id="<?php echo $campo1 ?>">
								<?php 
								$query = $pdo->query("SELECT * FROM indicadores WHERE id_plataforma = 6  ORDER BY descricao ASC");
								$res = $query->fetchAll(PDO::FETCH_ASSOC);
								for($i=0; $i < @count($res); $i++){
									foreach ($res[$i] as $key => $value){	}
										$id_item = $res[$i]['id'];
